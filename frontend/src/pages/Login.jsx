...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: isMobile ? "column" : "row", background: "#0f172a", overflowX: "hidden", width: "100%", maxWidth: "100vw", boxSizing: "border-box" }}>
      {/* Left brand panel - hidden entirely on mobile to save space */}
      {!isMobile && (
        <div style={{ width: 420, flexShrink: 0, background: "linear-gradient(160deg,#1e1b4b 0%,#0f172a 60%)", display: "flex", flexDirection: "column", justifyContent: "center", padding: "48px 40px", borderRight: "1px solid #1e293b" }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 700, color: "#f8fafc", lineHeight: 1.2, margin: "0 0 12px" }}>Smart Preparation,<br/>Better Results</h1>
          <p style={{ color: "#475569", fontSize: 14, lineHeight: 1.7, marginBottom: 36 }}>AI mock interviews tailored just for you.</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[["🤖","AI-Powered Questions","Role-specific, generated by Gemini AI"],["🎤","Voice Interviews","Live speech-to-text answer submission"],["📊","Detailed Analytics","Track your progress and skill breakdown"]].map(([icon,t,d]) => (
              <div key={t} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                <span style={{ fontSize: 18, flexShrink: 0 }}>{icon}</span>
                <div><p style={{ color: "#e2e8f0", fontSize: 13, fontWeight: 500, margin: "0 0 2px" }}>{t}</p><p style={{ color: "#475569", fontSize: 12, margin: 0 }}>{d}</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Mobile compact header - shown only on mobile, replaces the brand panel */}
      {isMobile && (
      ...
      {/* Right form panel */}
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "24px 20px" : 40, width: "100%", boxSizing: "border-box" }}>
        <div className="page-enter" style={{ width: "100%", maxWidth: 380 }}>
          <h2 style={{ fontSize: isMobile ? 22 : 26, fontWeight: 700, color: "#f8fafc", margin: "0 0 6px" }}>Welcome back!</h2>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                ...
                <Link to="/forgot-password" style={{ color: "#6366f1", fontSize: 12, textDecoration: "none" }}>Forgot Password?</Link>
              </div>
              <Input name="password" type="password" placeholder="••••••••" value={form.password} onChange={handleChange} onKeyDown={handleKeyDown} required />
              {capsLock && <p style={{ color: "#f59e0b", fontSize: 12, margin: "6px 0 0", display: "flex", alignItems: "center", gap: 4 }}>⚠ Caps Lock is on</p>}
            </div>

            <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
              <input type="checkbox" style={{ accentColor: "#6366f1" }} />
              <span style={{ color: "#475569", fontSize: 13 }}>Remember me</span>
            </label>
          ...
          <p style={{ color: "#334155", fontSize: 13, textAlign: "center" }}>
            ...
            <Link to="/register" style={{ color: "#6366f1", textDecoration: "none", fontWeight: 500 }}>Sign up</Link>
          </p>
        </div>
      </div>
    </div>
